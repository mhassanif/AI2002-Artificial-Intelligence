# Description: Solves the Course Crisis problem by finding the best swap (direct or chain) for each student
#              in text files from the Swapping folder, maximizing satisfactions while prioritizing older
#              students (21i- > 22i- > 23i- > 24i-). Displays the best swap option for each student per file,
#              allowing multiple swaps if optimal. Chains are printed in one line. Uses a search-based approach
#              without built-in libraries.
#              Input format: roll current_course current_section desired (desired is section or course-section).
